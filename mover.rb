...
class App
  ...
  def self.find_be_host(hostname, nodes = [])
    ...
    Resolv::DNS.open do |dns|
      ...
      be_name = case cname
      when /(be.core.pw|local.worldoftanks.cn|acme.io)$/
        cname
      when /fe.core.pw$/
        cname.gsub "fe.core.pw", "be.core.pw"
      when /worldoftanks.cn$/
        cname.gsub "worldoftanks.cn", "local.worldoftanks.cn"
      else
        #rr = Resolv.getname(Resolv.getaddress(hostname)).gsub("fe.core.pw", "be.core.pw") rescue nil
        zone = '.be.core.pw'
        begin
...
parser = OptionParser.new do |opts|
  ...
  opts.on("-a", "--app APP", "App name to process") { |app| options[:app] = app }
...
App.find_by_name options[:app] do |path, app|
  app_realm = path[0]
  next if options[:realm] && !(app_realm =~ /#{options[:realm]}/)
  app_name = path.compact.join('-').blue
  hostname = app['http']['host']
  if hostname =~ /acme.io$/
    LOG.info("app") { "#{app_name} is already in acme.io " + "(#{hostname} -> #{Resolv.getaddress(hostname)})".yellow }
    next
  end
  be_host = App.find_be_host hostname, app['nodes_list'].select { |n| n['master'] }.map { |n| n['fqdn'] }
  be_hostname = hostname.split('.').first + '.acme.io'
  unless be_host
    LOG.error("app") { "failed to get be hostname for #{app_name.red}" }
    next
  end
  LOG.info("app") { "#{app_name}, version " + "#{app["components_redefines"][path[-1]]["version"]}".green }
  App.change_http_host be_hostname, path, app["http"]
  App.commit_changes path[0], options[:apply]
  if options[:apply_dns]
    PowerDNS.add_record name: be_hostname, content: be_host, apply: options[:apply_dns], site: :net
    PowerDNS.add_record name: be_hostname, content: be_host, apply: options[:apply_dns], site: :cn
  end
  if options[:apply_certs]
    app["nodes_list"].map { |node| Hiera.add_certs node["fqdn"], "acme.io", options[:certs_owner], options[:apply_certs] } if app["http"]["ssl"]
  end
  ip = []
  ip[0] = Resolv.getaddress(hostname)
  ip[1] = Resolv.getaddress(be_hostname) rescue ""
  ip.map! { |i| i.split('.')[0..-2].join('.') + '.' + i.split('.')[-1].red rescue "" }
  LOG.debug("app") { "#{hostname} -> #{be_hostname} -> #{be_host} | #{ip[0]} -> #{ip[1]}" }
  LOG.warn("app") { "pushing configs to server" }
  system "fab -R #{path.compact.join('-')} push_configs:path=conf/nginx/#{ ',force=True' if options[:force] }" if options[:apply]
  #system "fab -R #{path.compact.join('-')} push_configs:path=app/src/wgnc_ui/settings_local.py#{ ',force=True' if options[:force] }" if options[:apply]
  system "fab -R #{path.compact.join('-')} nginx.reload_nginx" if options[:apply]
  #system "fab -R #{path.compact.join('-')} service_restart" if options[:apply]
end